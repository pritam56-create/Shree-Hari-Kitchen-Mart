...
export function validateContent(value: unknown, confirmed: boolean, imageReviewed = true) {
  if (!confirmed) throw new Error('Vegetarian compliance confirmation is required.');
  ...
  if (scan(value)) throw new Error('Content violates the vegetarian-only store policy.');
  if (!imageReviewed) throw new Error('A staff member must inspect every image before publication.');
}
