...
import {validateContent,ContentPolicyError} from '../../../lib/policy';
import {cartQuote,startCheckout,confirmCheckout,productInclude,adminProductInclude,releaseExpired} from '../../../lib/checkout';
...
import {adminAuth,adminFor,authorizeAdmin,requestMeta} from '../../../lib/admin-security';
import {adminModule} from '../../../lib/admin-modules';
import {beginAdminAudit,finishAdminAudit} from '../../../lib/admin-audit';
import {maintenance} from '../../../lib/maintenance';
import {restockReturn,completeRefund} from '../../../lib/refunds';
...
async function handler(req:NextRequest,{params}:{params:Promise<{path:string[]}>}){
 ...
 if(path==='internal/maintenance'){if(method!=='POST'||!process.env.COMMERCE_JOB_TOKEN||hash(req.headers.get('authorization')||'')!==hash('Bearer '+process.env.COMMERCE_JOB_TOKEN))throw new HttpError(403,'Forbidden');return ok(await maintenance());}
 ...
 if(path==='payments/webhook'){
  ...
  if(event.event==='refund.processed'){const remote=event.payload?.refund?.entity;if(remote?.id){const refund=await db.refund.findUnique({where:{gatewayRefundId:remote.id},include:{payment:true}});if(refund&&remote.status==='processed'&&remote.amount===paise(refund.amount)&&remote.payment_id===refund.payment.gatewayPaymentId)await completeRefund(refund.id,remote.id);}}
 ...
 if(path.startsWith('admin/auth/'))return await adminAuth(req,path.split('/')[2],body);
 ...
 if(path==='content'&&method==='GET'){const now=new Date();return ok(await db.contentEntry.findMany({where:{active:true,AND:[{OR:[{startsAt:null},{startsAt:{lte:now}}]},{OR:[{endsAt:null},{endsAt:{gt:now}}]}]},select:{id:true,kind:true,slug:true,title:true,body:true,imageUrl:true,mobileImageUrl:true,ctaText:true,ctaLink:true,priority:true},orderBy:{priority:'asc'},take:100}));}
 if(path.startsWith('questions/')&&method==='GET'){return ok(await db.question.findMany({where:{productId:path.split('/')[1],answers:{some:{}}},select:{id:true,body:true,answers:{select:{id:true,body:true,createdAt:true}}},orderBy:{createdAt:'desc'},take:50}));}
 ...
 if(path==='auth/login'){
  ...
  const user=await db.user.findFirst({where:{OR:[{email:v.email.toLowerCase()},{phone:v.email}],roles:{every:{roleId:'CUSTOMER'},some:{roleId:'CUSTOMER'}}}});
 ...
 if(path==='auth/forgot'){
  ...
  if(u&&!await db.userRole.count({where:{userId:u.id,roleId:{not:'CUSTOMER'}}})){const rawToken=token();await db.authToken.create({data:{id:hash(rawToken),userId:u.id,purpose:'RESET',expiresAt:new Date(Date.now()+1800000)}});await db.outbox.create({data:{kind:'RESET_EMAIL',payload:json({email,url:`${process.env.APP_URL}/reset-password?token=${rawToken}`})}});}
 ...
 const staff=path.startsWith('admin/')?await adminFor(req):null;
 const user=staff||(await userFor(req))!;
 ...
 authorizeAdmin(staff!,resource,method,body,resourceId);
 const extended=await adminModule(req,staff!,resource,resourceId,body);if(extended)return extended;
 ...
 if(resource==='products'){
  if(method==='GET')return ok(resourceId?await db.product.findUnique({where:{id:resourceId},include:adminProductInclude}):await db.product.findMany({include:adminProductInclude}));
 ...
 if(resource==='orders'){
  ...
  return ok(await transaction(async tx=>{const order=await tx.order.findUniqueOrThrow({where:{id:resourceId},include:{items:true,payment:true}});try{checkTransition(order.status,v.status);}catch(error){throw new HttpError(409,(error as Error).message);}
 ...
 if(resource==='refunds'){
  ...
  await db.refund.update({where:{id:refund.id},data:{gatewayRefundId:result.id,status:'PROCESSING'}});return ok(result.status==='processed'?await completeRefund(refund.id,result.id,user.id):await db.refund.findUnique({where:{id:refund.id}}));
 }
 ...
 throw new HttpError(404,'Unknown endpoint');
  ...
  if(error instanceof ContentPolicyError)return NextResponse.json({error:error.message},{status:400});
...
async function auditedHandler(req:NextRequest,context:Parameters<typeof handler>[1]){
 let audit:Awaited<ReturnType<typeof beginAdminAudit>>=null;
 try{if(!['/api/payments/webhook','/api/internal/maintenance'].includes(req.nextUrl.pathname))checkOrigin(req);audit=await beginAdminAudit(req);}catch(error){if(error instanceof HttpError)return NextResponse.json({error:error.message},{status:error.status});if(error instanceof SyntaxError)return NextResponse.json({error:'Invalid JSON'},{status:400});console.error('Audit initialization failed');return NextResponse.json({error:'Administrative audit could not be recorded'},{status:503});}
 const response=await handler(req,context);
 if(audit)try{await finishAdminAudit(audit,response.status);}catch{console.error('Admin audit completion pending',audit.id);}
 return response;
}
export {auditedHandler as GET,auditedHandler as POST,auditedHandler as PATCH,auditedHandler as DELETE};
